...
def removeTweetElems(str): #removes tweet elements like hashtags, RTs, and links
	regex = re.compile("^(RT)|(@[^\s]*\s?)|(#[^\s]*\s?)|(https://[^\s]*[\s]?)*")
	return regex.sub('', str);
    
def cleanCorpus(str):
	return removePunctuation(removeTweetElems(removeStops(str))).lower();
...
def scrape(keyTerm, n):
    ...
    return [removeTweetElems(tweet) for tweet in tweets]


#deprecated old frequency implementation, replaced with NLTK
